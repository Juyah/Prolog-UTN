...
votosTotales(Partido, Votos):-
	distinct(Partido,votos(Partido,_,_)),
	findall(Voto,votos(Partido, _, Voto), VotosEnLista),
	sum_list(VotosEnLista, Votos).

decidida(Provincia):-
	partidoMuchosVotosEnProvincia(Partido, Provincia),
	not((partidoMuchosVotosEnProvincia(OtroPartido, Provincia),Partido \= OtroPartido)).

partidoMuchosVotosEnProvincia(Partido, Provincia):-
	padron(Provincia, VotosTotales),
	votos(Partido, Provincia, VotosPartido),
	VotosPartido > VotosTotales * 30 / 100.
/*
decidida(Provincia):-
	padron(Provincia, Padron),
	findall(Voto,votos(_,Provincia, Voto), VotosEnLista),
	votoMayoritario(VotoMayor, VotosEnLista, Padron),
	not((votoMayoritario(VotoMayor2, VotosEnLista, Padron), VotoMayor \= VotoMayor2)).

votoMayoritario(VotoMayor, VotosEnLista, Padron):-
	member(VotoMayor,VotosEnLista),
	VotoMayor > (Padron * 30 / 100).
*/
